<template>
	<el-card shadow="never">
		...
		<el-timeline v-if="experienceList.length">
			<draggable handle=".drag-handle" @end="moved">
				...
				>
					...
					<template v-else>
						<el-descriptions>
							<template slot="title">{{item.company}}</template>
							<div class="no-print" slot="extra">
							...
							<el-descriptions-item label="担任职位" :span="1">
								<div v-html="item.jobs"></div>
							</el-descriptions-item>
							<el-descriptions-item label="所属行业" :span="2">
								<div v-html="item.business"></div>
							</el-descriptions-item>
							<el-descriptions-item label="工作描述" :span="3">
		...
		<el-empty v-else description="请添加工作经历"></el-empty>
	</el-card>
</template>
<script lang="ts">
